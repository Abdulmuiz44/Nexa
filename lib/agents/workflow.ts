...
async function nodePublishContent(state: WorkflowState): Promise<Partial<WorkflowState>> {
    ...
    const socialMediaService = new SocialMediaService(state.userId);
    ...
    for (const platform of state.toolkits) {
        ...
        try {
            const result = await socialMediaService.post(platform as any, content);

            if (result.success && result.platformPostId) {
                postIds.push(result.platformPostId);
                newLog.push(`✓ Posted to ${platform}: ${result.platformPostId}`);
            } else {
                newLog.push(`✗ Failed to post to ${platform}: ${result.error || 'Unknown error'}`);
            }
        } catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    return {
        ...
        executionLog: [
            ...newLog,
            postIds.length > 0 ? `✓ Publishing complete: ${postIds.length} posts published` : 'ℹ Publishing skipped or failed',
        ],
    };
}
...
async function nodeAnalytics(state: WorkflowState): Promise<Partial<WorkflowState>> {
    ...
    for (const platform of state.toolkits) {
        try {
            // Retrieve recent aggregated metrics
            const summary = await analyticsEngine.getAnalyticsSummary(state.userId, 'day');
            metrics[platform] = summary.platforms[platform] || { message: 'No metrics yet' };
        } catch (error) {
