...
const features = [
  {
    ...
    description: "Our Mistral-powered AI creates engaging posts, tweets, and replies in multiple tones, tailored to your brand voice. Save hours on content creation.",
  },
  {
    icon: Calendar,
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-6 px-2 sm:grid-cols-2 sm:gap-8 sm:px-0 lg:grid-cols-3">
          {features.map((feature, idx) => (
            <Card
              key={idx}
              className="group flex h-full flex-col rounded-3xl border-border bg-card/50 p-6 transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:border-primary/50 hover:shadow-neon sm:p-8"
            >
              <div className="relative inline-flex p-3 rounded-lg bg-primary/10 mb-5 group-hover:bg-primary/20 transition-colors w-fit">
              ...
              <p className="flex-grow text-sm leading-relaxed text-muted-foreground sm:text-base mb-4">{feature.description}</p>
              <Link href="/features" className="mt-auto flex items-center gap-1 text-sm font-semibold text-primary transition-colors hover:text-primary/80 group/link">
                Learn More
